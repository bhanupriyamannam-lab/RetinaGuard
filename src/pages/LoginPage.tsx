    ...
    <div className="min-h-screen w-full bg-slate-50 flex flex-col justify-between items-center p-4 sm:p-6 lg:p-8 font-sans selection:bg-blue-600 selection:text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-[460px] my-auto">
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xl shadow-slate-900/5 p-7 sm:p-9 transition-all">
          ...
          {/* Inline Error Banner */}
          {errorMessage && (
            <div className="mt-5 p-3 rounded-xl bg-rose-50 border border-rose-200/80 flex items-start gap-2.5 text-xs text-rose-700 animate-in fade-in slide-in-from-top-1 duration-150" role="alert">
          ...
          <button
            ...
            disabled={isLoading || isGoogleLoading}
            className="w-full h-11 rounded-xl border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs transition-colors flex items-center justify-center gap-2.5 shadow-2xs cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isGoogleLoading ? (
